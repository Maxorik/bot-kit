...
function addNewOrder(payload, tableName) {
    db.serialize(() => {
        let sqlInsert = `INSERT INTO ${tableName} (order_id, order_state, product, user, price, name, count)
                                VALUES ("${payload.order_id}", "${payload.state}", "${payload.product}", "${payload.user}", "${payload.price}", "${payload.name}", "${payload.count}")`;
